<?php 
while ($rollin = $getinfteef->fetch_assoc()) {
    $uzeexx = $rollin['username'];
    
    $get_phot = $db->query("SELECT * FROM users WHERE username='$uzeexx'");
    $roz = $get_phot->fetch_assoc();
    $id = $roz['id'];
    $unex = $roz['username'];
    $uni = $roz['vac3'];
    $maix = $roz['email'];
...
$(document).ready(function() {
    ...
    var follo = "<?php echo $counx; ?>";
    $.post('counset.php?UN=<?php echo $unex; ?>',{follo: follo},function(data) {
        $("#addsez").html(data);
    });
});
});

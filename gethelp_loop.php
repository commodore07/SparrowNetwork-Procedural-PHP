...
while ($row = $getposts->fetch_assoc()) {
						$id = $row['id'];
						$followe = $row['provider'];
                                                $rceivex = $row['receiver'];
...
$(document).ready(function() {
    ...
    document.getElementById("popconfid").innerHTML="<?php echo $id; ?>"; 
    document.getElementById("confixpop").innerHTML = "<center><img src='./loader2.gif'></center>";      
...
?>                       
                        ...
                        if($paycaant > 0){
                        ?>
                        <a id="deqopop<?php echo $id; ?>" title="Confirm pop" class="text-green" style="cursor: pointer;" data-toggle="modal" data-target=".confirmpop">Confirm POP</a>
                        <?php 
                        }
                        ?>
                        <br>
...
?>
<span id="loadcoux<?php echo $id; ?>" style="font-family: digital-clock-font; font-size: 18px; font-weight: 700; color: blue;"><?php echo "$ago_lapse"; ?></span><br>
<?php 
}
?>
...
$(document).ready(function() {
    ...
    var repcax = "";
    document.getElementById("terqoi").innerHTML=repcax;        
...
if($user != "support"){
?>
<a id="chatit<?php echo $id; ?>" data-toggle="modal" data-target=".chatmod" title="Send a private message" class="text-green" style="cursor: pointer;">Private message</a>
<?php
}
?>    
...
<script>
$(document).ready(function() {
    $('#revblopop<?php echo $id; ?>').click(function() {
    document.getElementById("revconfid").innerHTML="<?php echo $id; ?>"; 
    document.getElementById("revixpop").innerHTML = "<center><img src='./loader2.gif'></center>";      
    var revivid = "<?php echo $id; ?>";
    $.post('conf_disp.php',{rejid: revivid},function(data) {
        $("#revixpop").html(data);
    });
});
});
...
<a id="revblopop<?php echo $id; ?>" data-toggle="modal" data-target=".reversepop" title="Reverse & Block" class="text-green" style="cursor: pointer;">Reverse & Block</a>    
<?php
}
?>                                        

<?php
if($paycaant > 0 && $rceivex == "$user" && $user != "support"){
...
?>
                                    ...
                                    $(document).ready(function(){
                                        $.ajaxSetup({cache: false});
                                      setInterval(function(){
                                          $('#loadcoux<?php echo $id; ?>').load('loadzcounx.php?id=<?php echo $id; ?>');
                                      }, 1000);  
                                    });
                                    </script>     

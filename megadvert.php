...
<?php 
$getmag = $db->query("SELECT * FROM posts WHERE added_by='$username' ORDER BY id DESC") or die($db->error());
$info = $getmag->num_rows;

$getposts = $db->query("SELECT * FROM posts WHERE added_by='$username' ORDER BY id DESC LIMIT 10" ) or die($db->error());
include("while_megadvert.php");
?>
</div>    
                ...
                $(document).ready(function(){
                    ...
                    $("#readmore").on("click", function () {
                         ...
                         $.post("ajax_megadvert.php?U=<?php echo $username; ?>",{load:loadclick},function(data){
                           $('.images').append(data);
                           $('.procin').hide();
                           $('#readmore').show();
                         });

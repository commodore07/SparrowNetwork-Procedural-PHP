...
<?php 
$getmag = $db->query("SELECT * FROM my_chat WHERE sender = '$user' OR receiver= '$user' ORDER BY time_sent DESC") or die($db->error());
$info = $getmag->num_rows;

$get_vazz = $db->query("SELECT * FROM my_chat WHERE sender = '$user' OR receiver= '$user' ORDER BY time_sent DESC LIMIT 10");
...
<script>
                ...
                $(document).ready(function(){
                    ...
                    $("#readmore").on("click", function () {
                         ...
                         $.post("ajax_msg.php",{load:loadclick},function(data){
                           $('#images').append(data);
                           $('.procin').hide();
                           $('#readmore').show();
                         });

...
<?php
$getmed = $db->query("SELECT * FROM my_expenses WHERE username = '$user' AND tran_date='$follo' ORDER BY id DESC") or die($db->error());
$infoc = $getmed->num_rows;
while ($row = $getmed->fetch_assoc()) {
$exp_amtert2 = $row['exp_amt'];
/////////////// TOTAL AMOUNT
////////////////
$pzeeex = array ($exp_amtert2);
$valuux = array_sum ($pzeeex);
$totalxz +=$valuux;
$gr_total = "$totalxz";
$gra_totaa = number_format($gr_total);
////////////////////////
}

$total_eev = $db->query("SELECT * FROM my_expenses WHERE username = '$user' AND tran_date='$follo' ORDER BY id DESC LIMIT 10") or die($db->error());
...
?>
                ...
                $(document).ready(function(){
                    ...
                    $("#redova").on("click", function () {
                         ...
                         $.post("ajaxexp.php?TD=<?php echo $tandee; ?>",{load:loadkiv},function(data){

...
<?php 
$getmag = $db->query("SELECT * FROM notify WHERE username='$user' AND emp1!='$user' ORDER BY id DESC") or die($db->error());
$info = $getmag->num_rows;

$getinftee = $db->query("SELECT * FROM notify WHERE username='$user' AND emp1!='$user' ORDER BY id DESC LIMIT 10" ) or die($db->error());
include("note_loop.php");
?>                    
...
<script>
                ...
                $(document).ready(function(){
                    ...
                    $("#readmore").on("click", function () {
                         ...
                         $.post("ajax_note.php",{load:loadclick},function(data){
                           $('#images').append(data);
                           $('.procin').hide();
                           $('#readmore').show();
                         });

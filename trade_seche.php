...
<?php 
$getmag = $db->prepare("SELECT * FROM tradefair WHERE username=? AND name LIKE CONCAT('%',?,'%') ORDER BY id DESC") or die($db->error());
$getmag->bind_param("ss", $username, $temooo);
$getmag->execute();
$getmag = $getmag->get_result();

$info = $getmag->num_rows;

$getposts = $db->prepare("SELECT * FROM tradefair WHERE username=? AND name LIKE CONCAT('%',?,'%') ORDER BY id DESC LIMIT 10" ) or die($db->error());
$getposts->bind_param("ss", $username, $temooo);
$getposts->execute();
...
?>
                ...
                $(document).ready(function(){
                    ...
                    $("#readmore").on("click", function () {
                         ...
                         $.post("ajaxtradesarch.php?U=<?php echo $username; ?>&term=<?php echo $temooo; ?>",{load:loadclick},function(data){
                           $('.images').append(data);
                           $('.procin').hide();
                           $('#readmore').show();
                         });

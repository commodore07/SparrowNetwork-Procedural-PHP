...
if($pstat == "on"){
?>                  
                  <center><img src="sparrow_verify.jpg" style="width: 50px; height: 50px;" alt="" class="img-responsive profile-photo" /></center>
<?php 
}
?>                  
                </div>
...
<?php 
$getmag = $db->query("SELECT * FROM posts WHERE empty1='tradefairpic' ORDER BY id DESC") or die($db->error());
$info = $getmag->num_rows;

$getposts = $db->query("SELECT * FROM posts WHERE empty1='tradefairpic' ORDER BY id DESC LIMIT 10" ) or die($db->error());
include("while_activate.php");
?>
</div>    
                ...
                $(document).ready(function(){
                    ...
                    $("#readmore").on("click", function () {
                         ...
                         $.post("ajax_tradeshow.php?U=<?php echo $username; ?>",{load:loadclick},function(data){
                           $('.images').append(data);
                           $('.procin').hide();
                           $('#readmore').show();
                         });

...
<?php
//// Check if a cashier
   $lim_xeea = $db->query("SELECT * FROM users WHERE username='$username'");
   $rollin = $lim_xeea->fetch_assoc();
   $fexeea = $rollin['me_cashier']; 
   $uzee = $rollin['username']; 
if($fexeea != "" && $uzee == "$user"){
    echo "<meta http-equiv=\"refresh\" content=\"0; url=cashier\">";
	exit();
} else if($fexeea != "" && $fexeea == "$user"){
    echo "<meta http-equiv=\"refresh\" content=\"0; url=$user.trade\">";
	exit();
} else if($fexeea != "" && $fexeea != "$user"){
    echo "<meta http-equiv=\"refresh\" content=\"0; url=$fexeea.trade\">";
	exit();
} else {
    //// DO NOTHING
}  

$lim_bee = $db->query("SELECT * FROM users WHERE username='$user'");
   $rollbee = $lim_bee->fetch_assoc();
   $fexbee = $rollbee['me_cashier']; 
if($fexbee == "$username"){
    echo "<meta http-equiv=\"refresh\" content=\"0; url=cashier\">";
	exit();
}   
...
if($username == "$user" && $user != ""){
?>                    
                    <div class="text-green" data-toggle="modal" data-target=".tradeupz" style="text-align: center; margin: 10px; cursor: pointer;"><i class="fa fa-shopping-cart"></i> Upload tradefair Item</div>   
...
$delxa = $db->real_escape_string($delxa); 
echo "<meta http-equiv=\"refresh\" content=\"0; url=track_tran?track_id=$delxa\">";  
}
?>            
